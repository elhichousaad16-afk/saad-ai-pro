...
import {
  ...
  Layers, Calculator, Circle, Sun, Moon
} from "lucide-react";
...
// كائن الألوان الحالي — يُحدَّث (mutate) بمكانه عند تبديل الوضع الليلي/النهاري بدل استبداله بالكامل،
// لأن عشرات المكوّنات بالملف تقرأ منه مباشرة كمتغيّر عام.
const C = { ...LIGHT_THEME };
...
const TD_QUOTE_REFRESH_INTERVAL_MS = 900000; // 15 دقيقة — 7 رموز × 96 دورة/يوم = 672 credit، آمن ضمن 800/يوم

// مؤشرات وسلع إضافية عبر نفس Twelve Data (دفعة ثانية منفصلة بفاصل أطول لتبقى الميزانية اليومية آمنة):
// 6 رموز × دورة كل 70 دقيقة ≈ 20 دورة/يوم × 6 = 120 credit — يترك هامشًا كافيًا فوق ميزانية الفوركس (672).
const TD_QUOTE_SYMBOLS_2 = {
  SPX500: "SPX",
  NAS100: "NDX",
  US30: "DJI",
  GER40: "DAX",
...
const TD_QUOTE_2_REFRESH_INTERVAL_MS = 4200000; // 70 دقيقة
const TD_QUOTE_2_STARTUP_DELAY_MS = 75000; // تأخير بدء أولي لتفادي تصادم مع دفعة الفوركس بنفس الدقيقة
...
const LIVE_MANAGED_IDS = new Set([
  ...
  ...Object.keys(TD_QUOTE_SYMBOLS_2),
  ...Object.keys(BINANCE_SYMBOLS),
  ...Object.keys(GOLDAPI_SYMBOLS),
]);

// ارتباطات معروفة وعامة بين الأصول الرئيسية (تقريبية، مبنية على سلوك تاريخي معروف للأسواق، وليست محسوبة لحظيًا)
// موجبة قوية = يتحركان بنفس الاتجاه غالبًا | سالبة قوية = يتحركان بعكس بعض غالبًا
const CORRELATION_PAIRS = [
  ["EURUSD", "GBPUSD", 0.85], ["EURUSD", "USDCHF", -0.9], ["GBPUSD", "USDCHF", -0.78],
  ["AUDUSD", "NZDUSD", 0.88], ["AUDUSD", "USDCAD", -0.7], ["EURUSD", "AUDUSD", 0.6],
  ["USDJPY", "USDCHF", 0.55], ["XAUUSD", "XAGUSD", 0.85], ["XAUUSD", "USDJPY", -0.5],
  ["BTCUSD", "ETHUSD", 0.82], ["BTCUSD", "SOLUSD", 0.75], ["ETHUSD", "SOLUSD", 0.78],
  ["BTCUSD", "BNBUSD", 0.7], ["BTCUSD", "XRPUSD", 0.65],
  ["SPX500", "NAS100", 0.92], ["SPX500", "US30", 0.88], ["NAS100", "US30", 0.83],
  ["WTI", "USDCAD", -0.55],
];
function getCorrelation(id1, id2) {
  const found = CORRELATION_PAIRS.find(([a, b]) => (a === id1 && b === id2) || (a === id2 && b === id1));
...
function AIChart({ bars, levels, decimals, live, height = 420 }) {
  ...
  useEffect(() => {
    ...
    addLine(levels.resistance, C.dim, "مقاومة");
    addLine(levels.support, C.dim, "دعم");
    if (levels.sl) addLine(levels.sl, C.red, "SL");
    addLine(levels.tp1, C.green, "TP1");
    if (levels.tp2) addLine(levels.tp2, C.green, "TP2");
    if (levels.tp3) addLine(levels.tp3, C.green, "TP3");
    addLine(levels.entryLow, C.cyan, "دخول", false);
    if (levels.entryHigh !== undefined && Math.abs(levels.entryHigh - levels.entryLow) > 1e-9) addLine(levels.entryHigh, C.cyan, "دخول", false);
...
/* ============================== الشريط العلوي ============================== */
// الأسواق التقليدية (فوركس/معادن/مؤشرات/سلع) مغلقة من جمعة 22:00 UTC لحد أحد 22:00 UTC. الكريبتو دايمًا مفتوحة.
function getMarketStatus() {
  const now = new Date();
  const day = now.getUTCDay(); // 0=أحد, 5=جمعة, 6=سبت
  const hour = now.getUTCHours();
  const closed = (day === 5 && hour >= 22) || day === 6 || (day === 0 && hour < 22);
  return { closed };
}

function TopBar({ overallSentiment, themeMode, onToggleTheme }) {
  const [now, setNow] = useState(new Date());
  const [showMarketInfo, setShowMarketInfo] = useState(false);
  useEffect(() => { const t = setInterval(() => setNow(new Date()), 1000 * 30); return () => clearInterval(t); }, []);
  const { closed } = getMarketStatus();
  return (
    <div className="flex items-center justify-between px-6 py-4 sticky top-0 z-20"
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="relative">
          <button onClick={() => setShowMarketInfo((v) => !v)}
            className="flex items-center gap-1.5 text-xs px-3 py-1 rounded-full font-bold"
            style={{
              background: closed ? `${C.red}1A` : `${C.green}1A`,
              color: closed ? C.red : C.green,
              border: `1px solid ${closed ? C.red : C.green}44`,
            }}>
            <span className="inline-block rounded-full" style={{ width: 7, height: 7, background: closed ? C.red : C.green }} />
            {closed ? "الأسواق التقليدية مغلقة" : "الأسواق التقليدية مفتوحة"}
          </button>
          {showMarketInfo && (
            <div className="absolute top-full mt-2 right-0 z-30 w-72 rounded-xl p-3 text-xs shadow-lg"
              style={{ background: C.bgPanel, border: `1px solid ${C.border}`, color: C.softWhite }}>
              <div className="font-bold mb-2" style={{ color: C.goldBright }}>حالة الأسواق الآن</div>
              <div className="flex items-center justify-between mb-1.5">
                <span>الفوركس / المعادن / المؤشرات / السلع</span>
                <b style={{ color: closed ? C.red : C.green }}>{closed ? "مغلقة" : "مفتوحة"}</b>
              </div>
              <div className="flex items-center justify-between mb-2">
                <span>العملات الرقمية</span>
                <b style={{ color: C.green }}>مفتوحة دائمًا 24/7</b>
              </div>
              <p style={{ color: C.dim }}>الأسواق التقليدية تُغلق من الجمعة 22:00 إلى الأحد 22:00 (بتوقيت UTC) كل أسبوع.</p>
            </div>
          )}
        </div>
      </div>
      <div className="flex items-center gap-3">
        <button onClick={onToggleTheme} aria-label="تبديل الوضع الليلي/النهاري"
          className="w-8 h-8 rounded-full flex items-center justify-center"
          style={{
            background: themeMode === "dark" ? "#0B2A5B" : `${C.gold}1A`,
            border: `1px solid ${themeMode === "dark" ? "#1E4C9A" : C.gold + "44"}`,
            color: themeMode === "dark" ? "#8FC1FF" : C.gold,
          }}>
...
function AssetDetail({ model, signal, price, changePct, inWatchlist, toggleWatch, goToPlan, live, lastUpdated, alerts, addAlert, deleteAlert }) {
  ...
  const effectivePlan = useMemo(() => (aiPlan ? {
    source: "ai",
    bias: aiPlan.decision,
    confidence: aiPlan.confidence,
    entryLow: aiPlan.entry_low,
    entryHigh: aiPlan.entry_high,
    sl: aiPlan.sl,
    tp1: aiPlan.tp1,
    tp2: aiPlan.tp2,
    tp3: aiPlan.tp3,
  ...
  async function runFullAI() {
    setAiLoading(true); setAiError(null); setAiResult(null); setAiDecision(null); setAiPlan(null);
    try {
...
1) راجع كل استراتيجية واحدة تلو الأخرى من القائمة أعلاه (الـ 18 استراتيجية بالاسم بدون استثناء)، واذكر لكل واحدة: منطبقة الآن أو غير منطبقة، مع سبب مختصر جدًا (سطر واحد لكل استراتيجية). لا تتجاهل أو تحذف أي استراتيجية من القائمة، حتى لو غير منطبقة.
...
بعد كل ما سبق، أضف بالضبط كتلة JSON واحدة في نهاية ردك (بدون أي نص بعدها)، بين ثلاث علامات backtick وكلمة json، بهذا الشكل الحرفي (أرقام فقط بدون فواصل أو رموز عملة):
\`\`\`json
{"decision":"BUY|SELL|WAIT","confidence":75,"entry_low":0,"entry_high":0,"sl":0,"tp1":0,"tp2":0,"tp3":0,"rr":0}
\`\`\`
استخدم نفس الأرقام بالضبط التي ذكرتها بالفقرة 4 أعلاه داخل كتلة الـ JSON.`;
      ...
      if (jsonMatch) {
        try {
          ...
          if (obj && obj.decision) parsedPlan = obj;
        } catch (e) {
          console.warn("تعذر تحليل خطة الذكاء الاصطناعي JSON:", e.message);
        }
      }
      const displayText = jsonMatch ? text.slice(0, jsonMatch.index).trim() : text;
      setAiResult(displayText);
      setAiDecision(parsedPlan ? parsedPlan.decision : detectDecision(displayText));
        ...
        <div className="flex items-center gap-3">
          <div>
            ...
            {live && REFRESH_MS_BY_ID && (
              <div className="flex items-center gap-1.5 mt-1.5" title={`آخر تحديث: منذ ${fmtDuration(secondsSinceUpdate)} · التحديث القادم خلال: ${fmtDuration(secondsUntilNext)}`}>
                <span className="inline-block rounded-full animate-pulse" style={{ width: 8, height: 8, background: "#F5C518", boxShadow: "0 0 0 3px #F5C51822" }} />
                <span className="text-[10px]" style={{ color: C.dim, fontFamily: FONT_MONO }} dir="ltr">{fmtMMSS(secondsUntilNext)}</span>
              </div>
            )}
            {!live && (
      ...
      <Panel className="p-4 mb-5">
        <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
          <div className="flex items-center gap-2">
            ...
            {effectivePlan.source === "ai" ? (
              <span className="text-[10px] font-bold px-2 py-0.5 rounded" style={{ color: C.cyan, background: `${C.cyan}1A`, border: `1px solid ${C.cyan}44` }}>
              ...
              <span className="text-[10px] font-bold px-2 py-0.5 rounded" style={{ color: C.dim, background: `${C.dim}14`, border: `1px solid ${C.border}` }}>
                ⚙️ نظام آلي — شغّل التحليل الشامل أدناه ليصبح الرسم من الذكاء الاصطناعي
              </span>
            )}
          </div>
        ...
        <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${C.border}` }}>
          <AIChart bars={signal.ohlcBars} levels={effectivePlan} decimals={model.decimals} live={live} height={420} />
        </div>
        <p className="text-[11px] mt-2" style={{ color: C.dim }}>
          الخطوط المرسومة: منطقة الدخول (سماوي)، وقف الخسارة (أحمر)، الأهداف (أخضر)، والدعم/المقاومة (رمادي متقطع). الشموع تقريبية مبنية من أسعار الإغلاق فقط وليست بيانات Open/High/Low حقيقية بالكامل.
        </p>
      </Panel>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
        ...
        <Panel className="p-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <PlanStat label="منطقة الدخول" value={`${fmtNum(Math.min(effectivePlan.entryLow, effectivePlan.entryHigh), model.decimals)} - ${fmtNum(Math.max(effectivePlan.entryLow, effectivePlan.entryHigh), model.decimals)}`} />
            <PlanStat label="وقف الخسارة" value={effectivePlan.sl ? fmtNum(effectivePlan.sl, model.decimals) : "—"} color={C.red} />
            <PlanStat label="الهدف الأول" value={fmtNum(effectivePlan.tp1, model.decimals)} color={C.green} />
            <PlanStat label="الهدف الثاني" value={effectivePlan.tp2 ? fmtNum(effectivePlan.tp2, model.decimals) : "—"} color={C.green} />
            <PlanStat label="الهدف الثالث" value={effectivePlan.tp3 ? fmtNum(effectivePlan.tp3, model.decimals) : "—"} color={C.green} />
            <PlanStat label="نسبة R:R" value={effectivePlan.rr ? `1:${fmtNum(effectivePlan.rr, 1)}` : "—"} color={C.gold} />
          </div>
...
function Watchlist({ models, signals, prices, changes, watchlist, toggleWatch, openAsset, liveStatus }) {
  ...
  for (let i = 0; i < items.length; i++) {
    for (let j = i + 1; j < items.length; j++) {
      const corr = getCorrelation(items[i].id, items[j].id);
      if (corr !== null && Math.abs(corr) >= 0.7) {
        correlationWarnings.push({ a: items[i], b: items[j], corr });
      }
    }
  }
  return (
    <div>
      ...
      {correlationWarnings.length > 0 && (
        <div className="rounded-xl p-4 mb-5" style={{ background: `${C.gold}0D`, border: `1px solid ${C.gold}33` }}>
          <div className="text-xs font-bold mb-2 flex items-center gap-1.5" style={{ color: C.gold }}>
            <AlertTriangle size={14} /> تنبيه ترابط بين أصولك (مخاطرة مضاعفة محتملة)
          </div>
          <div className="space-y-1.5">
            {correlationWarnings.map((w, i) => (
              <p key={i} className="text-xs" style={{ color: C.softWhite }}>
                <b>{w.a.nameAr}</b> و<b>{w.b.nameAr}</b> {w.corr > 0 ? "مرتبطان إيجابيًا بقوة" : "مرتبطان سلبيًا بقوة"} (≈{fmtNum(w.corr, 2)}) —
                {w.corr > 0 ? " فتح صفقتين بنفس الاتجاه عليهما فعليًا يضاعف مخاطرتك على نفس الفكرة." : " فتح صفقتين متعاكستين عليهما قد يلغي أحدهما تأثير الآخر."}
              </p>
            ))}
          </div>
        </div>
...
/* ============================== الإعدادات ============================== */
function Settings({ settings, setSettings, resetData, restartTour }) {
  ...
  return (
    <div>
      ...
      <Panel className="p-5 mb-5">
        ...
        <div className="mt-4">
          <button onClick={restartTour} className="px-4 py-2 rounded-xl text-xs font-bold" style={{ border: `1px solid ${C.gold}66`, color: C.gold }}>
            🎬 إعادة عرض الجولة التعريفية
          </button>
        </div>
      </Panel>
      <Panel className="p-5 mb-5">
...
const ONBOARDING_STEPS = [
  { title: "أهلًا فيك بـ SAAD AI Trading 👋", body: "منصة شخصية لتحليل الأسواق المالية بالذكاء الاصطناعي — أسعار حية، تشارتات احترافية، تحليل فني، وإدارة مخاطر. خلينا ناخد جولة سريعة (دقيقة وحدة بس)." },
  { title: "القائمة الجانبية", body: "من هون بتنقّل بين كل الأقسام: لوحة التحكم، الفوركس، الذهب، الكريبتو، الماسح الذكي، دفتر التداول، وغيرها. اضغط أي قسم لتفتحه." },
  { title: "لوحة التحكم", body: "بتعرضلك كل الأصول دفعة وحدة — الشارة الخضراء LIVE تعني سعر حقيقي فعلي، والرمادية SIMULATED تعني بيانات محاكاة للعرض فقط." },
  { title: "صفحة أي أصل", body: "دوس على أي بطاقة أصل لتفتح تشارت TradingView حقيقي + تشارت مرسوم عليه خطوط الدخول/وقف الخسارة/الأهداف، وزر لتشغيل تحليل الذكاء الاصطناعي الكامل." },
  { title: "التنبيهات والمراقبة", body: "أضف أي أصل لقائمة المراقبة ⭐، أو حط تنبيه سعري 🔔 ليوصلك إشعار حقيقي من المتصفح لما يوصل السعر لهدفك." },
  { title: "دفتر التداول والاختبار الخلفي", body: "سجّل صفقاتك الحقيقية وتابع أداءك بمنحنى فعلي، أو جرّب استراتيجية على بيانات تاريخية حقيقية بصفحة الاختبار الخلفي." },
  { title: "تذكير أخير مهم ⚠️", body: "هذه أداة لدعم القرار وتعليم التحليل، وليست نصيحة مالية. لا يوجد ولن يوجد أي ضمان لتحقيق ربح. راجع صفحة الإعدادات لتفاصيل أكثر حول مصادر البيانات." },
];
...
function OnboardingTour({ onFinish }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ background: "rgba(10,14,23,0.6)" }}>
      <div className="w-full max-w-md rounded-2xl p-6" style={{ background: C.bgPanel, border: `1px solid ${C.border}` }}>
        <div className="flex items-center gap-1.5 mb-4">
          {ONBOARDING_STEPS.map((_, i) => (
            <span key={i} className="h-1.5 flex-1 rounded-full" style={{ background: i <= step ? C.gold : C.border }} />
          ))}
        </div>
        <h3 className="font-extrabold text-base mb-2" style={{ color: C.softWhite }}>{s.title}</h3>
        <p className="text-sm leading-relaxed mb-6" style={{ color: C.dim }}>{s.body}</p>
        <div className="flex items-center justify-between">
          <button onClick={onFinish} className="text-xs font-semibold" style={{ color: C.dim }}>تخطي الجولة</button>
          <div className="flex gap-2">
            {step > 0 && (
              <button onClick={() => setStep((v) => v - 1)} className="px-4 py-2 rounded-xl text-xs font-bold" style={{ border: `1px solid ${C.border}`, color: C.dim }}>
                السابق
              </button>
            )}
            <button
...
// زر عائم "رجوع للأعلى" لتسهيل التنقل بالصفحات الطويلة
function ScrollToTopButton() {
  ...
  return (
    <button
      ...
      className="fixed bottom-6 left-6 z-50 w-11 h-11 rounded-full flex items-center justify-center shadow-lg"
      style={{ background: C.gold, color: "#FFFFFF", border: `2px solid ${C.goldBright}` }}>
...
export default function App() {
  ...
  // دفعة ثانية: مؤشرات وسلع إضافية عبر Twelve Data (بفاصل زمني أطول وتأخير بدء لتجنب تصادم الحصة مع دفعة الفوركس)
  useEffect(() => {
    let cancelled = false;
    async function fetchLive2() {
      try {
        const symbols = Object.values(TD_QUOTE_SYMBOLS_2).join(",");
        const res = await fetch(`/.netlify/functions/quotes?symbols=${encodeURIComponent(symbols)}`);
        if (!res.ok) { console.warn("Twelve Data (دفعة 2) function error:", res.status); return; }
        const data = await res.json();
        if (cancelled) return;
        if (data && data.code) { console.warn("Twelve Data API error (دفعة 2):", data.code, data.message); }
        const nextPrices = {};
        const nextChanges = {};
        const nextStatus = {};
        Object.entries(TD_QUOTE_SYMBOLS_2).forEach(([id, sym]) => {
          const q = data[sym];
          if (q && q.close && !q.code) {
            nextPrices[id] = parseFloat(q.close);
            if (q.percent_change !== undefined) nextChanges[id] = parseFloat(q.percent_change);
        ...
        console.warn("Twelve Data (دفعة 2) fetch failed:", e.message);
      }
    }
    const startTimer = setTimeout(() => {
      fetchLive2();
      const t2 = setInterval(fetchLive2, TD_QUOTE_2_REFRESH_INTERVAL_MS);
      (fetchLive2)._interval = t2;
    }, TD_QUOTE_2_STARTUP_DELAY_MS);
    return () => {
      cancelled = true;
      clearTimeout(startTimer);
      if (fetchLive2._interval) clearInterval(fetchLive2._interval);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    async function loadHistoryFor(id, sym, delayMs) {
      ...
      try {
        const res = await fetch(`/.netlify/functions/history?symbol=${encodeURIComponent(sym)}&outputsize=500`);
  ...
  useEffect(() => {
    (async () => {
      const seen = await loadKey("onboarding-done", false);
  ...
  useEffect(() => {
    (async () => {
      const savedMode = await loadKey("theme-mode", "light");
      if (savedMode === "dark") {
        Object.assign(C, DARK_THEME);
        setThemeMode("dark");
  ...
  function toggleTheme() {
    ...
    Object.assign(C, next === "dark" ? DARK_THEME : LIGHT_THEME);
    setThemeMode(next);
    setThemeVersion((v) => v + 1);
    saveKey("theme-mode", next);
  }
  ...
  return (
    <div key={themeVersion} dir="rtl" style={{ fontFamily: FONT_AR, background: C.bgDeep, minHeight: "100vh" }} className="flex">
      ...
      <div className="flex-1 min-w-0">
        <TopBar overallSentiment={overallSentiment} themeMode={themeMode} onToggleTheme={toggleTheme} />
        <main className="p-6">
          {view === "dashboard" && (
          ...
          {view === "settings" && <Settings settings={settings} setSettings={setSettings} resetData={resetData} restartTour={() => setShowOnboarding(true)} />}
        </main>
      </div>
      <ScrollToTopButton />
      {showOnboarding && (
        <OnboardingTour onFinish={() => { setShowOnboarding(false); saveKey("onboarding-done", true); }} />
      )}
    </div>
  );
}
